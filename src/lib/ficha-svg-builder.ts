/**
 * Client-side SVG builder for VIVA MOBILI product sheets.
 * Extracted from the server-side generate-svg route for real-time preview.
 * No Node.js dependencies — pure string manipulation for browser use.
 */
...
/**
 * Build a professional VIVA MOBILI product sheet as SVG string.
 * Can be used client-side for live preview rendering.
 */
export function buildFichaSvg(data: CopilotFurnitureData): string {
  const W = 794;
  const H = 1123;
  const M = 40;
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const cy = viewStartY + row * (cellH + 28);
  ...
  // ── Design Highlights ──
  let annY = viewStartY + 2 * (cellH + 28) + 20;
...
/**
 * Build a compact ficha SVG for inline preview (scaled down).
 * Returns an SVG string optimized for embedding in the editing view.
 */
export function buildFichaPreviewSvg(data: CopilotFurnitureData): string {
  return buildFichaSvg(data);
}
